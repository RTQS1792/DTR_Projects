...
uint16_t sAddress = 0x2222;
uint8_t sCommand = 0x22;
uint8_t sRepeats = 1;
...
void receive_ir_data() {
    if (IrReceiver.decode()) {
        ...
        IrReceiver.resume();
        if(IrReceiver.decodedIRData.address == 0x2222 && IrReceiver.decodedIRData.command == 0x22){
          digitalWrite(12, HIGH); // Receives it's own signal
        }else if(IrReceiver.decodedIRData.address == 0x1111 && IrReceiver.decodedIRData.command == 0x11){
          digitalWrite(2, HIGH); // Receives partner's signal
        }
    }
}
